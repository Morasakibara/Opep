    ...
    <div className="space-y-8">
      <AnimatedMount animation="slide-up" durationMs={300} className="flex justify-between items-end">
        ...
        <button onClick={loadTickets}
          className="bg-surface_container_high text-on_surface px-4 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-surface_container active:scale-[0.97] transition-all border border-charcoal_border">
      ...
      <AnimatedMount animation="slide-up" durationMs={400} className="grid grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <div key={i} className="glass-card rounded-2xl p-5 active:scale-[0.98] transition-all">
      ...
      {error && (
        <div className="glass-card rounded-2xl p-6 flex items-center justify-between border-error_red/30">
          ...
          <button onClick={loadTickets} className="px-4 py-2 bg-primary text-on_primary text-sm font-bold rounded-xl hover:brightness-110 active:scale-[0.97] transition-all">Réessayer</button>
        </div>
      )}
        ...
        <div className="glass-card rounded-3xl overflow-hidden">
          <div className="p-5 border-b border-charcoal_border flex items-center gap-3">
            <button onClick={() => setFilter('all')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all active:scale-[0.97] ${filter === 'all' ? 'bg-primary text-on_primary' : 'text-on_surface_variant hover:text-on_surface'}`}>Tous</button>
            <button onClick={() => setFilter('valid')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all active:scale-[0.97] ${filter === 'valid' ? 'bg-primary text-on_primary' : 'text-on_surface_variant hover:text-on_surface'}`}>Valides</button>
          </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="divide-y divide-charcoal_border">
              {filtered.map((ticket, i) => (
                <div key={ticket.id} className="p-5 flex items-center gap-4 hover:bg-primary/5 hover:translate-x-0.5 transition-all group cursor-pointer"

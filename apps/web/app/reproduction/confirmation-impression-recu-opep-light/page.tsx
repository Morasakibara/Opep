...
/**
undefined
 * Route: /reproduction/confirmation-impression-recu-opep-light
 */
export default function ConfirmationImpressionRecuOpepLightReproductionPage() {
  const [activeTab, setActiveTab] = useState('home');
  const [showPrintModal, setShowPrintModal] = useState(false);
  return (
    <div className="min-h-screen bg-[#111316] text-[#e2e2e6] overflow-x-hidden">
      {/* SECTION: Mockup Layout */}
      <header className="flex justify-between items-center px-margin-desktop w-full sticky top-0 z-40 bg-surface-container-lowest h-16 border-b border-surface-variant no-print">
...
<div className="flex items-center gap-gutter border-l border-surface-variant pl-gutter">
...
<div className="w-8 h-8 rounded-full bg-surface-variant overflow-hidden border border-surface-variant">
<img alt="User Profile" className="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBj_tm38KxuBaYv0ypL4EOYQw-Mo2sJH-jU_bq_RWN3X7qSne9swKn7YnXhPRVHD6o1WxwGtVnFNGs1AtUSBTtPOe6BiALwPqNDanVjWnb1nN78dhdKVecKmok6S_yjvTiZWahxsnBpErr30GYe_nlf5te3QyAIGUs7fhpt9W-TgTUTN5E-xgl6uq9QRrivW6taGYOGjGXG6tEixVa4iVkeSp0h4nPO3Lgx2lqhwwwb7rOfs5wlTI8kSgY4vaFX09e_gYYpKb4hEC8"/>
</div>
</div>
</div>
</header>
...
<aside className="hidden md:flex flex-col h-[calc(100vh-64px)] w-[280px] bg-surface-container-low border-r border-surface-variant p-gutter gap-unit no-print">
...
<div>
<p className="font-label-caps text-label-caps text-on-surface font-bold">OPEP Admin</p>
<p className="text-[10px] text-on-surface-variant">Management Portal</p>
</div>
</div>
<nav className="flex-1 space-y-1">
<a className="flex items-center gap-unit p-3 text-on-surface-variant hover:bg-surface-variant hover:text-on-surface rounded-lg transition-all" href="#">
...
<span className="font-label-caps text-label-caps font-semibold">POS Terminal</span>
</a>
<a className="flex items-center gap-unit p-3 text-on-surface-variant hover:bg-surface-variant hover:text-on-surface rounded-lg transition-all" href="#">
...
<div className="mt-auto border-t border-surface-variant pt-gutter space-y-1">
<a className="flex items-center gap-unit p-3 text-on-surface-variant hover:bg-surface-variant rounded-lg transition-all" href="#">
...
<section className="flex-1 relative overflow-y-auto bg-surface-container p-margin-desktop md:p-12 flex items-center justify-center">
...
<button className="w-full md:w-auto flex items-center justify-center gap-unit px-8 py-4 bg-primary text-on-primary font-title-md text-title-md rounded-xl hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all active:scale-95 group" onClick={() => setShowPrintModal(true)}>
<span className="material-symbols-outlined group-hover:rotate-12 transition-transform" data-icon="print">print</span>
<span>Imprimer le Reçu</span>
</button>
<button className="w-full md:w-auto flex items-center justify-center gap-unit px-8 py-4 border border-outline-variant bg-surface-container-lowest text-on-surface font-title-md text-title-md rounded-xl hover:bg-surface-container-high transition-all active:scale-95">
<span className="material-symbols-outlined" data-icon="add">add</span>
<span>Nouvelle Transaction</span>
</button>
</div>
<div className="mt-12 grid grid-cols-2 gap-gutter max-w-sm">
<div className="p-gutter glass-card rounded-xl shadow-sm">
...
<span className="font-title-md text-title-md text-on-surface">Espèces</span>
</div>
</div>
<div className="p-gutter glass-card rounded-xl shadow-sm">
<p className="font-label-caps text-label-caps text-on-surface-variant mb-1">Caissier ID</p>
<p className="font-title-md text-title-md text-on-surface">POS-YAO-04</p>
</div>
</div>
</div>
...
<span className="font-label-caps text-label-caps uppercase">Aperçu avant impression</span>
</div>
...
<p className="text-center font-bold text-sm mb-1 uppercase">Douala ➔ Yaoundé</p>
...
<div className="w-full h-full bg-neutral-50 flex items-center justify-center relative overflow-hidden">
<div className="grid grid-cols-8 gap-px w-full h-full opacity-60">

</div>
<span className="absolute inset-0 flex items-center justify-center font-bold text-[8px] bg-white/80 px-1 text-center">QR CODE VALIDATION</span>
</div>
</div>
